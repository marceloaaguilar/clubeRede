...
export default function Cart() {
  ...
  return (
    <>
      ...
      <div
        className={`fixed top-0 right-0 bottom-0 z-50 w-96 bg-neutral-800 dark:bg-neutral-800 border-l border-neutral-700 dark:border-neutral-700 transform transition-transform duration-300 ${
        ...
        <div className="flex flex-col h-full">
          ...
          <header className="p-2 flex justify-between items-center border-b border-neutral-700 dark:border-neutral-700">
            <h2 className="text-xl font-semibold text-white dark:text-white">Carrinho</h2>
            <button
              onClick={() => setIsOpen(false)}
              className="text-neutral-400 hover:text-gray-700 dark:text-neutral-400"
          ...
          <div className="w-full h-full flex flex-col">
            ...
            <div className="flex-1 overflow-auto pb-36
              ...
              {items.length > 0 ? items.map((item, index) => (
                <div key={index} className="border border-gray-600 dark:border-gray-600 rounded-lg m-4 p-4 select-none">
                  <div className="flex justify-between items-center">
                    <div>
                      <h2 className="text-xl font-bold text-white dark:text-white">{item.title}</h2>
                      <p className="text-sm text-gray-300 dark:text-gray-300 mt-1">{item.description}</p>
                      <p className="text-xl text-gray-100 dark:text-gray-100 mt-2 font-bold">R$ {item.paymentValue.toString().replace(".", ',')}</p>
                      <span className="text-gray-300">Quantidade: {item.quantity || 0}</span>
                    </div>
                ...
                <div className="p-4">
                  <p className="text-gray-600 dark:text-gray-600">Seu carrinho está vazio.</p>
                </div>
              )}
            </div>
      ...
      {openModalDeleteItem && (
        <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50">
            <div className="bg-neutral-800 dark:bg-neutral-800 rounded-md p-6 max-w-sm w-full shadow-lg">
              ...
              <div className="flex justify-end gap-3">
                <button
                  className="px-4 py-2 rounded bg-neutral-700 dark:bg-neutral-700 text-gray-300 dark:text-gray-300 hover:bg-gray-300 dark:hover:bg-neutral-600"
